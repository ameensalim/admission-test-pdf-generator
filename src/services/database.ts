...
import { candiateInput, Candidate, Settings } from "../types";
...
export const searchCandidates = async (query: string): Promise<Candidate[]> => {
  ...
  const { data, error } = await supabase
    ...
    .select("*")
    .or(
      `name.ilike.%${lowercaseQuery}%,form_no.ilike.%${lowercaseQuery}%,token_no.ilike.%${lowercaseQuery}%,place.ilike.%${lowercaseQuery}%`
    )
    .order("created_at", { ascending: false });
...
function mapToCandidateModel(dbRecord: any): Candidate {
  return {
    ...
    name: dbRecord.name,
    dob: dbRecord.dob,
    contact_no: dbRecord.contact_no,
